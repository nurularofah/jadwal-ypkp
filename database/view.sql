...
/*!50001 VIEW `vjadwalmatakuliah` AS select `hj`.`kd_head_jadwal` AS `kode`,`dj`.`hari` AS `hari`,`m`.`nm_matkul` AS `matakuliah`,`m`.`sks` AS `sks`,concat(`dj`.`jammulai`,' - ',`dj`.`jamakhir`) AS `jam`,`r`.`nm_ruang` AS `ruangan`,`d`.`nm_dosen` AS `nm_dosen` from ((((`usb_jadwalmatkul` `hj` join `usb_detail_jadwalmatkul` `dj` on((`hj`.`kd_head_jadwal` = `dj`.`kd_head_jadwal`))) join `usb_matkul` `m` on((`dj`.`kd_matkul` = `m`.`kd_matkul`))) join `usb_ruang` `r` on((`dj`.`kd_ruang` = `r`.`kd_ruang`))) join `usb_dosen` `d` on((`dj`.`nip` = `d`.`nip`))) */;
...
-- Dump completed on 2014-06-24 10:24:12
